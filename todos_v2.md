...
## Pending Bash Differences ([])

- [] handle the `~` and possible `-` for cd
  - `cd ~` [done]
...
- [] bonus wildcard issues
 (`echo *`, `echo .*`). must be ordered correctly

- [] multi-line quoted strings in non-interactive mode (hanlde \n)
