...
/*   Updated: 2026/03/08 15:21:29 by haya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_matching(char *pattern, char *file_name)
{
    ...
    if(!*pattern || !*file_name)
        return (0);
    if(*pattern == '*')
        return is_matching(pattern + 1, file_name) || is_matching(pattern , file_name + 1) || is_matching(pattern + 1, file_name + 1);
    if(*pattern == *file_name)
...
char *append_astersk(char *result,char *pattern)
{
    ...
    while(1)
    {
        ...
        if(is_matching(pattern, current_file->d_name))
        {
            ...
            result = append_str(result, ft_strdup(current_file->d_name));
            i++;
        }  
    }
